...
describe('Store', () => {
  ...
  it('allows to place order by user, and admin can see created order', async () => {
    ...
    adminClient.store.getOrderById(placeOrder.id);
  });
});

const petWithStatus = (status: definitions['Pet']['status']) => {
  return {
    category: {
      id: 0,
      name: 'string'
